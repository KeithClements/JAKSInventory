...
> - 🟡 **Multi-PO single-shipment receiving — BACKEND done, UI pending.** The model already supported it (`POReceipt.vendor_id` + `POReceiptLine.po_id` per line). Fixed the service half: `create_receipt` now re-evaluates the status of EVERY PO touched (was last-PO-only) so a multi-PO receipt can close one PO while leaving another partial, + a cross-vendor guard (a receipt spans one vendor). Added `get_open_receivable_lines_for_vendor()` (open lines grouped by PO, drops settled/cancelled lines + non-receivable POs) for the screen. Test `tests/test_phase1_multi_po_receiving.py` (5 cases). **Still to build: the vendor-scoped "Receive Shipment" screen + route** (per-PO `/{po_id}/receive` stays; new multi-PO entry alongside it).
> - ⏳ **Open:** `competitor_part_number_norm` search; engine-picker Manufacturer table + reclassify-all; cross-vendor OEM xref-on-create dedup.
