...
> - ✅ **Markup-tier admin UI** (@28d9c97) — closes the exact gap the old preview route's own copy flagged ("Editing the tiers and the Activate toggle are pending Backend save routes"). New CRUD routes `POST /settings/pricing/tiers` (create/update/delete) + `POST /settings/pricing/toggle-active` (flips `markup_tiers_active`), inline in `routers/settings.py` matching `/settings/locations`'s own no-dedicated-service convention (`PricingService` is documented "stateless math — does NOT write to the database", so CRUD doesn't belong there). Full editable tier table + Activate-grid toggle on the Settings → Pricing tab, replacing the old read-only preview-only table (the Preview-impact dry-run itself is unchanged, still useful pre-activation). Delete is a real hard-delete (no FK from Product — tiers are matched by cost-bracket at read time, not referenced per-row). Test `tests/test_phase2_markup_tier_admin.py` (13).
> - ✅ **ProductApplication batch importer** (@28d9c97) — new `ProductImportService.import_applications(text, dry_run=True)`: CSV of sku/make/model/cpl/esn_range applied onto EXISTING products only (same never-create contract as every `pricing_update_*` mode). Idempotent on the model-docstring-locked dedup grain (product_id, engine_make, engine_model, cpl) — a repeat row refreshes `esn_range` instead of duplicating. Bulk-committed (one commit-or-rollback), not per-row, unlike `ProductService.add_application`'s own per-call commit — a real applications feed can be thousands of rows. New "Applications (Engine Fitment)" mode on the Import screen. **Bug found + fixed while wiring this:** `productImport()`'s `fetch()` also never stamped the CSRF header — the ENTIRE product importer (Full Import + Pricing Update + this new mode), the PRIMARY catalog-ingestion path for the whole 31k-SKU business, had 403'd since CSRF was added, silently. Fixed with the same local `_csrfToken()` helper used for the two Catalog Utilities buttons (§23.3 Phase 1). Test `tests/test_phase2_applications_importer.py` (11, incl. the CSRF regression).
>
> **Phase 2 is now FULLY COMPLETE — 6/6 items shipped, committed.** Full suite 2662 passed / 0 failed (2636 after #2/#3/#4/#6 @54ba0f4, +24 tests from #1/#5 @28d9c97; the 57 `test_visual_regression` failures seen mid-session were the documented flake from an orphaned dev-server process on :8000, unrelated). Commits: @54ba0f4 (valuation callout, dead-stock, reorder-to-PO, brand/manufacturer merge) → @28d9c97 (applications importer, markup-tier admin UI). Both on `backend/workflow-series-3`, not yet pushed.
>
> Two significant CSRF bugs found + fixed in passing while wiring this phase's UI (both pre-existing, unrelated to Phase 2's own scope): the two Catalog Utilities buttons on the Import screen, and — far more consequential — the ENTIRE product importer (Full Import + Pricing Update), the primary catalog-ingestion path for the whole business. All three had 403'd silently since CSRF was added (§21.3).
>
> **Incident:** live-verifying the brand-merge UI accidentally merged the real "PAI" brand into "Interstate-McBee" on the dev catalog (20,638 products). Recovered with explicit owner sign-off; see `[[live-verify-bulk-mutation-caution]]` memory for the new live-verify discipline going forward.

**Phase 3 — Core lifecycle hardening** *(Low–Med / Med–High)* — ✅ **ALL 5 ITEMS SHIPPED** (committed @d207644): default `warranty_require_esn=true` + blank-ESN warning; require/auto-create a VCR for single-core submits; standalone overdue-cores report; warranty search by ESN + replacement-parts tracking (`replacement_invoice_line_id`); ProductSerialNumber lookup screen.

> **Phase 3 progress (2026-07-01):**
> - ✅ **`warranty_require_esn` defaults ON + blank-ESN warning** — the submit gate (`warranty_service.py`) now defaults `"true"` both in code AND as a seeded, Settings-visible row (`routers/settings.py` DEFAULTS + VISIBLE_KEYS → Policy group), since PAI/IMB both reject blank-ESN vendor claims; flip it off in Settings for vendors that don't. Found+fixed in passing: a blank-ESN draft was permanently un-submittable (ESN was only settable at create time) — new `WarrantyService.set_esn()` + `POST /warranty/{id}/esn`, surfaced as an amber warning banner with an inline Save-ESN form on the claim workspace (copy states whether submission is actually blocked, via the live setting) + an ESN hint on the new-claim picker. Tests: `test_s21_warranty_interest.py` rewritten to pin the NEW default (blank blocked by default / allowed when off / set_esn unblocks); `test_warranty.py` / `test_returns_warranty_lifecycle.py` / `test_r2_warranty_esn.py` claim helpers now carry an ESN.
> - ✅ **Single-core submits ride the VCR ledger** — new `CoreService.submit_single_core_to_vendor()` wraps `create_vcr()`+`ship_vcr()` for a one-core batch, so the cores-list "Mark Shipped" path gets the same VCR document (expected credit, reconciliation, dispute trail, ISSUE_CREDIT_MEMO permission) as a batched box instead of silently bypassing it; a vendor-less core now errors with guidance instead of shipping into the void. The bare `submit_to_vendor()` transition stays as the low-level primitive (`ship_vcr` mirrors it per core; service tests unchanged). Route redirects to the VCR box document; the cores list already manages VCR'd cores on the Open VCRs card, so the decision flow needed zero changes. Test `tests/test_phase3_single_core_vcr.py` (7, incl. route-level).
> - ✅ **Standalone Overdue Cores report** — new `ReportService.get_overdue_cores()` derives from `get_core_charges_outstanding()` (single source of truth) keeping only past-deadline rows + `days_overdue` + customer phone (it's the "who do we call today" chase list). Route `GET /reports/overdue-cores` (+CSV), reports sub-nav entry, landing card (reuses the existing `core_overdue` count). Template mirrors `outstanding_cores.html` (reports family). Test `tests/test_phase3_overdue_cores_report.py` (7).
> - ✅ **Warranty search by ESN + replacement-parts tracking** — the Warranty Queue search now matches claim-level `esn` and per-line failed-part `serial_number`, and a search INCLUDES closed claims (an engine's warranty history outlives the work queue; new gray `Closed` chip renders only in search results — the un-searched queue still excludes them). `update_service_info()` accepts a per-line `replacement_invoice_number`, resolved server-side to that invoice's product line for the SAME product (unknown invoice / wrong part raise; blank clears) and stored in the previously-dormant `replacement_invoice_line_id` (+ new explicit-FK relationship — the class has two FKs into invoice_lines). Workspace service-info table gains a "Replacement Inv #" column (editable + read-only variants). Test `tests/test_phase3_warranty_esn_search_replacement.py` (9).
> - ✅ **Serial Number Lookup screen** — new read-only `GET /serials/` (router `app/routers/serials.py`, nav under Cores): search by serial or SKU; each row is the unit's chain of custody — received (PO + vendor + date via `POReceiptLine`), status chip, sold (invoice + customer + date via `InvoiceLine`), and warranty claims filed against that serial (case-insensitive `WarrantyClaimLine.serial_number` match). Three bulk loads, no per-row queries; capped at 200 rows with a refine hint; no-query view doubles as "recently recorded serials". *Reports-family styling (search + table), not a governed L2 list — same precedent as the receive-shipment screen.* Test `tests/test_phase3_serial_lookup.py` (5).
>
> **Phase 3 is FULLY COMPLETE — 5/5 items shipped @d207644.** Full suite **2702 passed / 0 failed**. Also fixed in passing (@27689d3, test-only): `test_phase2_dead_stock`'s sale fixture anchored on `utcnow().date()` while the service uses local `date.today()` — an off-by-one flake after ~6pm Mountain time; and the Phase-1 CSRF-stamp count pin (==2) that went stale when Phase 2 stamped `productImport()`'s fetch (correct count is 3). Housekeeping: killed the orphaned `:8000` uvicorn (PID 31960, verified `app.main:app` leftover from the prior session's preview) — port free, `preview_start` unblocked.

**Phase 4A — Inventory Counting (Audit · Cycle · Wall-to-Wall · Opening)** *(Med / High)* — ✅ **§24 Phase A SHIPPED 2026-07-07 (uncommitted)** — `count_sessions`/`count_lines` + `Product.bin_location`; `CountService` (create→open-snapshot→record→review→post→cancel) on top of `apply_stock_delta` (net-change: post `delta = counted − frozen`); `/counts` list + new-count wizard + status-dispatched workspace (mobile scan-to-find entry, blind hiding, recount-threshold gate, variance/accuracy review, FULL zero-uncounted confirm) + printable count sheet (PDF); perms `INVENTORY_COUNT`/`INVENTORY_COUNT_APPROVE`; nav entry. Tests `tests/test_count_sessions.py` (22, incl. the §24.8 must-cover matrix — mid-count-sale-preserved, blind-hides-book, recount thresholds, OPENING baseline, FULL zero-uncounted, permission split, re-post rejected, resync zero-drift). Full suite green; **live-verified end-to-end** on a copy of the real 78 k-row DB (COUNT-2026-0001: 12→10 short, 0→3 over, unchanged skipped; migration ran clean on the real DB). Phase B (ABC + scheduler) remains deferred → §23.4. *Original plan:* build **§24 Phase A** (manual counts of all four types + mobile scan entry + printable sheet + variance / record-accuracy reports) and time it to the **real-data go-live cutover the shop is now approaching** (transactions cleared 2026-07-06 for the live trial). *Why build it here:* (a) the **Opening** count type is the auditable way to load real starting on-hand — today only the raw `apply_physical_count` script exists; (b) its only dependencies — single-writer ledger (`apply_stock_delta`), moving-avg cost, multi-PO receiving — are already shipped in Phases 1–2; (c) it's file-disjoint from Phase 4 (new `count_sessions`/`count_lines` tables + `/counts` router), so it can run **before or in parallel** with it; (d) highest operational value — a parts shop can't run daily on inventory numbers it can't verify. **§24 Phase B (ABC + auto-scheduler) is deferred** to §23.4 — ABC needs trailing sales-velocity history and the 2026-07-06 clear left none; revisit after 60–90 days of daily use. *Success: load go-live opening on-hand via a supervised, per-line-audited Opening session (not a blind script); run a cycle count on one aisle while the counter keeps selling, and post it with mid-count sales preserved.*
...
ProductKit BOM explosion · ESNLookup vendor-portal API · messaging delivery (keep log-only) · eBay · multi-tenant infra · more AI/automation depth · Postgres/row-locks/session-revocation · **§24 Phase B — ABC classification + cycle-count auto-scheduler** (needs trailing sales-velocity history; the 2026-07-06 transaction clear left none — revisit after 60–90 days of daily use, same clock as the items below) · anything cosmetic — until Phase 0 is in and JAK's has run on it daily for 60–90 days.
...
## 24. Inventory Counting System — Audit · Cycle · Wall-to-Wall · Opening *(Phase A ✅ SHIPPED 2026-07-07, uncommitted — see §23.3 Phase 4A; Phase B deferred)*

**Goal.** Give the ERP a real physical-inventory capability. Today there is only a low-level bulk loader (`InventoryService.apply_physical_count(counts, dry_run)`) — no count *document*, no scope, no snapshot, no blind entry, no variance review, no post-approval. This section specifies a first-class **Count Session** that covers all four count styles the owner asked for.

### 24.1 — The model (locked): ONE entity, four `count_type`s

The gold-standard ERPs (NetSuite Inventory Count, Fishbowl Cycle Count, Odoo Inventory Adjustments) do NOT build four separate features. They build **one count document** with a type discriminator. Same lifecycle, same snapshot/variance/post machinery — only the *scope* and a couple of posting rules differ:

| `count_type` | What it is | Default scope | Posting nuance |
|---|---|---|---|
| `AUDIT` | Spot check / "is the number right?" | A handful of SKUs, ad-hoc | May be **count-only** (record variance, don't adjust) or post like a cycle count |
| `CYCLE` | Recurring slice so you rarely need a full count | ABC-scheduled subset (Phase B) or manual category/vendor/bin slice | Post over/short; uncounted lines are **skipped** (not zeroed) |
| `FULL` | Wall-to-wall physical inventory | Everything (a whole location) | Uncounted in-scope lines may be **zeroed** — but only behind an explicit confirm |
| `OPENING` | Initial inventory on hand at go-live | Everything, from a zero/empty baseline | Reason = `initial_inventory_load`; variance is vs 0 |

### 24.2 — Owner decisions LOCKED 2026-07-07 (interview)

1. **Bin locations = simple text field.** Add `Product.bin_location` (indexed string, e.g. `"A-03-B"`). Count sheets sort/group/filter by it so the count is *walkable*. Totals stay single-location — NOT going to full multi-bin per-location quantities (deferred, §24.9).
2. **Full count = keep operating (net-change method).** No blackout. Snapshot each in-scope product's `qty_on_hand` at session open (`frozen_qty` on the line); at post, apply **`delta = counted_qty − frozen_qty`** on the *live* ledger. This automatically preserves any sale/receipt booked mid-count, because the cache already absorbed that movement — see §24.4 (this is the correctness heart of the feature).
3. **Mobile-first entry.** Phone/tablet count screen: scan barcode / type SKU → jump to line → enter qty. Plus a printable count sheet fallback for dead-zone areas.
4. **ABC auto-scheduling — phased.** Manual scoping ships first (Phase A). ABC classification + auto-built cycle sessions land in Phase B.

Additional locked defaults (recommended, not separately interviewed — flip if wanted):
- **Blind by default.** Counter never sees `frozen_qty` during entry (kills the "just confirm the screen number" bias). Supervisor reveals in Review.
- **Recount threshold.** Lines whose variance exceeds a configurable qty **or** dollar threshold are flagged `NEEDS_RECOUNT` in Review and must be recounted or explicitly accepted before Post.
- **Count targets `qty_on_hand`, never `qty_committed`.** Committed-to-SO stock is still physically on the shelf until picked, so counters count it. We only ever adjust on-hand.

### 24.3 — Lifecycle (status machine)

`DRAFT` (scoping) → `OPEN` (snapshot taken, counting in progress) → `REVIEW` (variances computed, recounts flagged, blind reveal) → `POSTED` (ledger adjustments written, immutable) · `CANCELLED` (abandoned, zero ledger effect).

- Scope is editable in `DRAFT` only. Moving to `OPEN` resolves scope → concrete `count_lines` and **freezes the snapshot**.
- `POSTED` is terminal and immutable (re-post blocked → idempotent).

### 24.4 — Net-change reconciliation (why "keep operating" is safe) — the critical detail

At **open**: `frozen_qty` = `product.qty_on_hand` captured on the line (kept for the printed sheet + audit); `snapshot_at` stamped on the session. At **count entry**: `book_at_count` = `product.qty_on_hand` captured **at the instant the line is counted** — this, not the snapshot, is the reconciliation baseline. *(Corrects the original spec, which used `frozen_qty`; a code review caught that a counter counts the shelf as it is NOW, after any mid-count movement, so a sale booked before the count would otherwise be subtracted twice.)*
At **post**, for each counted line: `delta = counted_qty − book_at_count`, written via the existing **`InventoryService.apply_stock_delta`** (the single, canonical `qty_on_hand` writer — we do NOT add a second stock writer).

Because `book_at_count` is the live book at the moment of counting, the delta is exactly the *unexplained* discrepancy and stays correct in **every** movement ordering:

```
new_on_hand = current_on_hand + (counted − book_at_count)
```
- Sale booked **before** the shelf is counted → already in both `book_at_count` and physical `counted` → nets to zero (not double-subtracted).
- Sale booked **after** the count → preserved (delta is relative to the live cache, which already absorbed it).
- Genuine shrinkage → posts as the `counted − book_at_count` short/over.

No blackout required. `book_at_count` is the anti-double-count anchor. The `test_sale_before_count_not_double_subtracted` + `test_mid_count_sale_preserved` pair is the #1 correctness test (§24.8).

Ledger row per posted line: `transaction_type = INITIAL_COUNT`, `reference_type = "count_session"`, `reference_id = session.id`, `reason = cycle_count_over | cycle_count_short` (or `initial_inventory_load` for OPENING). Reason codes already exist in `AdjustmentReason` / `InventoryTxnType`. Counted stock is valued at existing average cost (a count sets quantity, not cost) — consistent with today's `apply_physical_count`. Nightly `resync_all_products` agrees after post (count writes real ledger rows), so no drift.

### 24.5 — Schema (Phase A)

**`count_sessions`** — id · `session_number` (`COUNT-2026-0001`, new number sequence in §13) · `count_type` · `status` · `name` · `location_id` (FK, default Main Shop) · `scope_json` (category_ids / vendor_ids / bin prefix / sku list / abc_class / is_active / ALL) · `blind` (bool) · `recount_threshold_qty` · `recount_threshold_value` · `snapshot_at` · `opened_by_id` · `posted_by_id` · `posted_at` · cached rollups (`line_count`, `counted_count`, `variance_qty_total`, `variance_value_total`) · `notes` · timestamps.

**`count_lines`** — id · `session_id` (FK) · `product_id` (FK) · snapshot fields captured at open: `sku`, `description`, `bin_location`, `frozen_qty`, `unit_cost` · `counted_qty` (nullable until counted) · `book_at_count` (live on-hand captured when the line is counted — the reconciliation baseline, §24.4) · `counted_by_id` · `counted_at` · `recount_count` · `needs_recount` (bool) · `line_status` (uncounted / counted / recount / posted / skipped) · `posted_txn_id` (FK → inventory_transactions, set on post) · `notes`. Variance is derived (`counted − book_at_count`).

**`products`** — add `bin_location` (indexed String). Migration + backfill blank.

### 24.6 — Service layer — `CountService`

- `create_session(count_type, scope, blind, thresholds, name)` → `DRAFT`.
- `open_session(id)` → resolve scope → snapshot one line per in-scope product (freeze qty/bin/cost), stamp `snapshot_at`, status `OPEN`. Gate `INVENTORY_COUNT`.
- `record_count(session_id, sku_or_product_id, counted_qty, by)` → upsert line `counted_qty` + `counted_at`; flag `needs_recount` if over threshold. This is the mobile hot path (HTMX per-line save).
- `add_found_line(session_id, sku)` → shelf item not in scope ("found stock") — snapshot it in.
- `to_review(id)` → compute variances + recount flags → `REVIEW`.
- `post_session(id)` → **one transaction**: for each counted line with variance ≠ 0, `apply_stock_delta(...)`; set `posted_txn_id` + `line_status=posted`; `FULL` may zero uncounted-in-scope (explicit `zero_uncounted=True` confirm) else skip+warn; audit each; status `POSTED`. Gate `INVENTORY_COUNT_APPROVE`.
- `cancel_session(id)`.
- Phase B: `refresh_abc()` (writes `Product.abc_class` from trailing usage value) · `build_cycle_session()` (auto-scope from ABC cadence + `last_counted_at`).

**Permissions (new):** `INVENTORY_COUNT` (create/scope/enter — broad, countable by floor staff) and `INVENTORY_COUNT_APPROVE` (review + post — manager/admin). Posting also implicitly satisfies the existing `INVENTORY_ADJUST` guarantees since it routes through `apply_stock_delta`.

**Reuse, don't fork:** the low-level `apply_physical_count` stays as a scripted bulk path; the OPENING count *type* is its supervised, auditable UI equivalent. `apply_stock_delta` remains the ONLY cache writer.

### 24.7 — Router / UI — `/counts`

- **List screen** — sessions with type + status chips, filters, "in progress / due" surfacing. Follows the jaks-ui-governance list pattern.
- **New-count wizard** — pick type → scope builder (category / vendor / bin range / SKU paste / ABC / ALL) → options (blind, thresholds) → open.
- **Session workspace** — header (status, progress bar X/Y counted, live variance summary), **Count** tab (mobile scan-to-find + qty stepper; desktop bulk grid + CSV/paste), **Review** tab (variance table, recount flags, blind reveal, $ impact by category/bin), **Post** action.
- **Print count sheet** (PDF, walkable by bin).
- **Reports** — per-session variance report; **inventory record accuracy %** KPI (zero-variance lines ÷ counted lines); uncounted-in-scope report.
- **Nav** — add "Counts" to the Inventory area (§4 navigation).

### 24.8 — Testability (must-cover)

1. **Mid-count sale preserved** (the net-change proof): open → sell 2 via a real invoice → count the shelf → post → final on-hand = counted + the −2. *The single most important test.*
2. Blind: entry payload/markup never exposes `frozen_qty` before Review/Post.
3. Recount threshold flags on both qty and $ limits.
4. Post is atomic (partial failure → full rollback; nothing half-written).
5. `OPENING` posts from an empty baseline with `initial_inventory_load` reason.
6. `FULL` won't zero uncounted-in-scope without the explicit confirm.
7. Permission split: `INVENTORY_COUNT` can enter but not post; `INVENTORY_COUNT_APPROVE` can post.
8. Idempotency: re-posting a `POSTED` session is rejected.
9. Nightly `resync_all_products` shows zero drift after a post.

### 24.9 — Build order

**Phase A — manual counts, all four types (ship first):** (1) schema + `Product.bin_location` + `COUNT-` number sequence + migration; (2) `CountService` create/open-snapshot/record/review/post/cancel on top of `apply_stock_delta`; (3) `/counts` list + new-count wizard + session workspace (desktop grid) + post; (4) mobile count entry + scan-to-find; (5) printable count sheet PDF; (6) variance + record-accuracy + uncounted reports; (7) the §24.8 test suite.

**Phase B — automation:** (8) ABC classification job + `Product.abc_class`; (9) cycle-count scheduler (auto-build sessions by ABC cadence + `last_counted_at`, "due for cycle count" surfacing); (10) record-accuracy KPI on the dashboard.

**Phase C — deferred (§24.9 do-not-build-yet):** serial-number reconciliation *within* a count · true multi-bin per-location quantities (upgrade path from the §24.2 bin text field) · handheld/dedicated-scanner hardware integration.

### 24.10 — When to build (sequenced 2026-07-07)

**Phase A = build next; time it to the real-data go-live cutover.** Slotted into the authoritative roadmap as **§23.3 Phase 4A** (ahead of / parallel to Phase 4 — the two are file-disjoint). The trigger is concrete: the 2026-07-06 "testing go-live clear" wiped all transactions and reset counters for the live trial, so the shop is at the exact moment where **loading real opening on-hand matters** — and the only tool for it today is the bare `apply_physical_count` script (no per-line audit, no variance review, no recount). Everything Phase A depends on (single-writer ledger, moving-avg cost, multi-PO receiving) already shipped in Phases 1–2, so there is no code reason to wait.

**Phase B = do NOT build yet (§23.4).** ABC classification ranks items by *trailing* usage value/velocity; the 2026-07-06 clear left zero sales history, so an ABC job run now would classify on empty data. Revisit after **60–90 days of daily use** (the same clock §23.5 sets for earning Option B). Until then, cycle/audit counts are scoped manually (category / vendor / bin / SKU) — which is all Phase A ships anyway.

**Net:** the counting *workflow* is a go-live-adjacent capability (build it into the cutover); the counting *automation* is a mature-daily-use capability (build it once the shop has history worth analyzing).
