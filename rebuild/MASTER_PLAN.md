...
**Phase 1 — Stabilize the spine** *(Med / High)* — multi-PO single-shipment receiving + fix last-PO-only status loop (`po_service.py:489`); `competitor_part_number_norm` column+index+listener+`_TARGETS` before loading ATL/HHP; `qbo_push_batch` → plain `def` (stop event-loop freeze); net `credit_balance` into `available_credit`; engine picker → Manufacturer table + `POST /products/reclassify-all` (dry-run); cross-vendor OEM xref check before creating a NEW product (stop PAI/IMB dup products → add vendor source instead). *Success: receive a 3-PO truck in one action; HHP-number search instant; IMB re-import of a PAI part adds a source not a 2nd product.*

**Phase 2 — Product & inventory cleanup** *(Med / High)* — ProductApplication batch importer (sku/make/model/cpl/esn_range); dead-stock report (`get_dead_stock`+route+template); reorder-to-PO from low-stock; brand merge + constrain brand/manufacturer to managed lists; markup-tier admin UI; inventory-valuation cost-source callout.
...
**🟠 HIGH:**
3. **New/special-order parts quote & sell at $0.00 until first receipt.** Estimated sell = `product.cost × markup` (`models/product.py:317-330`); `product.cost` (moving-avg COGS) is 0 until a receipt, even though `vendor_sources.vendor_cost` is known. After receiving, cost→$850 and price→$1105 (confirmed). **Fix:** fall back to preferred `vendor_sources.vendor_cost` when `product.cost==0` (and/or warn on a $0 line).
4. **No logout / no Account link in the UI; avatar hardcoded "K"/"Keith"** (`base.html:685-690`). `/logout` in zero templates; `/account` only in the conditional banner. **Fix:** avatar dropdown bound to `current_user` with Account + Sign out.

**🟡 MEDIUM:** taxable customers' quotes/invoices default to **"Exempt"** + `default_sales_tax_rate=0` (no tax ever applied; mislabels "0% rate" as "Exempt") — relates to §21.1 decision "quote-tax = default-from-customer + clerk-toggle" which is NOT honored on the document default · **"Today's Cash" = $0** after a same-day payment (UTC `payment_date` vs local "today" filter; for US-Mountain, payments after ~5pm local stamp "tomorrow") · dup-customer detection is **email-only, not phone** · **Inventory Valuation report ~27s** (renders all ~31k rows — paginate/summary) · **Google Fonts blocked app-wide by CSP** (style-src/font-src) · negative cost + invalid email accepted server-side (client-only guards).

**🟢 LOW:** raw-JSON 404 page · no `Cache-Control: no-store` on auth pages (back-button cache) · Alpine `x-trap` without the Focus plugin (warning every page) · login fields lack `required` · JAKS→Axle rebrand incomplete (page `<title>`s say "JAKS Inventory"; printed core slip says "JAK's Diesel" — consistent with [[axle-rebrand]]) · phantom "$0.00 < $0.00" core banner on the product form.
...
**Recommended sequencing:** the two CRITICAL one-liners + the $0-pricing fallback should slot ahead of §23.3 Phase 1 (they block the two most common daily actions). The tax-default and Today's-Cash items fold naturally into Phase 4 (reporting & controls).
