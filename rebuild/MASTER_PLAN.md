...
> - ⏳ **Open:** multi-PO single-shipment receiving (headline — needs new route + UI + `receive()` multi-PO status; `receive()` currently assumes "all lines belong to the same PO" `po_service.py:385`); `competitor_part_number_norm` search; engine-picker Manufacturer table + reclassify-all; cross-vendor OEM xref-on-create dedup.
