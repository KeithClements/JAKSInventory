...
from app.constants import (
    ...
    LineType, SOLineSource, SOPaymentMode, SOStatus,
)
...
class SalesOrderService(BaseService):
    ...
    def add_line(self, so_id: int, product_id: int | None, data: dict) -> SOLine:
        """
        Add a line to an open or partial SO.
        Writes SO_COMMITTED inventory transaction for product lines.
        """
        ...
        line = self._add_line_internal(so.id, {**data, "product_id": product_id}, sort_order)
        self.db.commit()
        return line
    ...
    def fulfill_and_invoice(
        ...
        Sequence:
          1. For each line: validate qty, release committed inventory (SO_RELEASED),
             update qty_fulfilled / qty_invoiced, mark BACKORDER if partially filled.
        ...
        for so_line_id, qty in line_quantities.items():
            ...
            if line.qty_remaining > 0:
                line.source = SOLineSource.BACKORDER
    ...
    def cancel_order(self, so_id: int, reason: str) -> None:
        """
    ...
    def _add_line_internal(self, so_id: int, data: dict, sort_order: int) -> SOLine:
        ...
        line = SOLine(
            ...
            source=SOLineSource.STOCK,
            sort_order=sort_order,
        )
        self.db.add(line)
        self.db.flush()

        # Commit inventory for product lines
        if line.line_type == LineType.PRODUCT and line.product_id and qty_ordered > 0:
            product = self.db.query(Product).filter(Product.id == line.product_id).first()
            if product:
