...
async def log_call_global(request: Request, db: Session = Depends(get_db)):
    ...
    customer = db.query(Customer).filter(Customer.id == int(customer_id_raw)).first()
    if not customer:
        return HTMLResponse(
...
def _parse_rows(raw_rows: list[dict[str, str]]) -> tuple[list[dict], list[dict]]:
    ...
    for i, row in enumerate(raw_rows, start=2):  # row 2 = first data row after header
        ...
        valid.append({
            ...
            "discount_pct": float(canonical.get("discount_pct", "0") or "0"),
            "interest_rate": float(canonical.get("interest_rate", "0") or "0"),
...
async def customer_import_preview(
    ...
    # Flag existing duplicates (by company name, case-insensitive)
    existing_names = {
        r[0].lower()
...
async def customer_import_confirm(
    ...
    """Write validated rows to DB. Skips exact company-name duplicates."""
    form = await request.form()
    ...
    try:
        ...
        return RedirectResponse("/customers/import?error=Invalid+data", status_code=303)
    ...
    try:
        for row in rows:
            if skip_dupes and row.get("company_name", "").lower() in existing_names:
                skipped += 1
                continue
            db.add(Customer(
