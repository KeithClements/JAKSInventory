...
- **6 failed (all non-functional)** — the 5 cosmetic `test_ui_lint.py` checks **+ 1** pre-existing `test_template_renders.py::test_w4_cost_variance_only_no_qty_over_narrative` (a brittle parallel-lane W-4 match-panel assertion; memory flags **do-not-chase**; not a money path). **X1's gate is "0 *functional* failures" — still met.** *(QA reconciled the count 2026-06-02; where other sections say "5 cosmetic reds," read "these 5 + the 1 non-functional W-4.")*
...
**Coverage gap — CLOSED 2026-06-02:** `tests/test_payments.py` (15 tests, QA `@75be7b8`) now nets the payment money-path — record · allocate across 2 invoices · reverse (reopens invoice + restores credit) · NSF fee · overpay → unapplied credit. **Ground-truth pinned by the tests:** overpayment parks as `Payment.amount_unallocated` (via `get_unapplied_balance`), it does **not** post to `customer.credit_balance` — the tests assert that so a future silent "auto-convert" can't slip in. *(Was: no dedicated payments file; exercised only indirectly.)*
...
| D-5 | **Payments depth** *(no dedicated auto-file — test hard)*: record → allocate across 2 invoices; **overpay** → prompts credit (never silent); **reverse** → reopens invoice + restores credit; **NSF** → marks payment + creates NSF fee; **card** → surcharge on the card portion | each behaves; balances reconcile; nothing silently swallowed | `tests/test_payments.py` (15) @75be7b8 | ✅ **PASS — owner manual 2026-06-02 + automated net CLOSED** (`tests/test_payments.py`, 15, QA): record · allocate-×2 · reverse · NSF fee · overpay→unapplied credit (parks as `amount_unallocated`, not `credit_balance`). |
